...
searchInput.addEventListener("input", (event) => {
    let query = event.target.value.toLowerCase(); // Get user input

    const filteredNames = animalNames
    .filter(animal => animal.name.toLowerCase().includes(query))
    .map(animal => animal.name)
    // let filteredAnimals = animalNames.filter(name => name.includes(query)); // Filter names

    handleSearchBar(filteredNames, query, animalNames);
});

// Function to display search results
function handleSearchBar(results, query) {
    menu.innerHTML = ""; // Clear previous results
    //to hide the search list menu when there's no input
    if(query.trim()===""){
        menu.style.display = "none";
        return;
    }
    //to show list when there's input
    ...
    results.forEach(name => {
        ...
        //to find the full animal object to show animal details
        let animal = animalNames.find(animal =>animal.name ===name)
        listItem.addEventListener("click", ()=>{
            if(animal){
                showAnimalDetails(animal)
            }else{
                console.error("Animal not found", name)
            }
        })
